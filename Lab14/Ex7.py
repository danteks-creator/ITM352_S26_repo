...
def main() -> None:
	...
	if not data_file.exists():
		raise FileNotFoundError(
			f"Could not find {data_file.name} in the same folder as Ex7.py."
		)
	...
	plt.tight_layout()
	plt.savefig("Ex7_3d_fare_miles_dropoff.png")
	plt.close()

	print(f"Rows plotted: {len(clean)}")
	print("Saved plot to Ex7_3d_fare_miles_dropoff.png")
